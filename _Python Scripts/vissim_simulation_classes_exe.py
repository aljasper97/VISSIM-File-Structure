...
class Vissim_Network:
    ...
    def initialize_network(self):
        ...
        inpx_file = os.path.join(self.network_path, self.network_name)
    ...
    def save_network_files(self,new_save_path=None):
        if new_save_path is None:
            inpx_file = os.path.join(self.network_path, self.network_name)
            layx_file = os.path.join(self.network_path, self.network_name.replace('inpx','layx'))
        else:
            inpx_file = os.path.join(new_save_path, self.network_name)
            layx_file = os.path.join(new_save_path, self.network_name.replace('inpx','layx'))
        
        time.sleep(5)  # Wait loop for Vissim to shut down
        self.vissim.SaveNetAs(inpx_file)
        #self.vissim.SaveLayout(layx_file) # only save layx if file has already run?
